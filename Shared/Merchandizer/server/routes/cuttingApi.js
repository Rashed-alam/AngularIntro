...
router.post('/all/:referenceId/:styleCode', (req, res, next) => {
    CuttingSchema.findOne({
        referenceId: req.params.referenceId,
        styleCode: req.params.styleCode
    }).then(function(a) {
        if (a != null) {
            var alldata = JSON.parse(JSON.stringify(a.cutting));
            var ref = a.referenceId;
            var styC = a.styleCode;
            var remark = a.remarks;
            for (i = 0; i < alldata.length; i++) {
                alldata[i].referenceId = ref;
                alldata[i].styleCode = styC;
                alldata[i].remarks = remark;
            }
            res.send(alldata);
        } else {
            var x = [];
